...
public class CartFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        binding.finalBuy.setOnClickListener(v -> {
            ...
            intent.putExtra("amount", finalPaymentAmount); // Subtotal
            intent.putExtra("shipping", finalPaymentAmount * 0.15); // Shipping cost (15%)
            intent.putExtra("discount", finalPaymentAmount * 0.1); // Discount (10%)
            intent.putExtra("product", (Serializable) list); // Pass the cart list
            intent.putExtra("quantity", list.size()); // Pass quantity
